...
// Account Merge Routes (Workplan Line 553: Merge duplicate accounts)
const mergeService = require('./merge.service');

const mergeAccountsSchema = Joi.object({
...
router.post(
  ...
  authenticate,
  requireAdmin(),
  validate(mergeAccountsSchema),
  async (req, res, next) => {
    try {
      const { sourceUserId, targetUserId } = req.body;
      const preview = await mergeService.previewMerge(sourceUserId, targetUserId);
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const result = await mergeService.mergeAccounts(
        sourceUserId,
        targetUserId,
        req.user.id,
        reason
